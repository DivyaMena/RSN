...
async def generate_report(input: ReportInput, request: Request):
    """Generate reports for different entity types with date range filtering"""
    await require_admin(request)
    ...
    if input.report_type == "enrollments":
        ...
        report_data["summary"]["total_count"] = len(batches)
        report_data["summary"]["total_enrollments"] = total_batch_enrollments
        ...
        report_data["summary"]["total_count"] = len(students)
    
    elif input.report_type == "tutors":
        # Get tutors with user info
        tutors_query = date_query.copy()
        tutors = await db.tutors.find(tutors_query, {"_id": 0}).to_list(None)
        
        for tutor in tutors:
            tutor_obj = Tutor(**tutor)
            user_doc = await db.users.find_one({"id": tutor_obj.user_id}, {"_id": 0, "name": 1, "email": 1})
            
            if user_doc:
                report_data["details"].append({
                    "tutor_code": tutor_obj.tutor_code,
                    "name": user_doc.get("name", "N/A"),
                    "email": user_doc.get("email", "N/A"),
                    "board_preference": tutor_obj.board_preference,
                    "classes_can_teach": ", ".join(map(str, tutor_obj.classes_can_teach)),
                    "subjects_can_teach": ", ".join(tutor_obj.subjects_can_teach),
                    "status": tutor_obj.status,
                    "approval_status": tutor_obj.approval_status,
                    "registered_on": tutor_obj.created_at.isoformat() if isinstance(tutor_obj.created_at, datetime) else tutor_obj.created_at
                })
        
        report_data["summary"]["total_count"] = len(tutors)
    
    elif input.report_type == "coordinators":
        # Get coordinators
        coordinators_query = {"role": "coordinator"}
        coordinators_query.update(date_query)
        
        coordinators = await db.users.find(coordinators_query, {"_id": 0, "password_hash": 0}).to_list(None)
        
        for coord in coordinators:
            report_data["details"].append({
                "user_code": coord.get("user_code", "N/A"),
                "name": coord.get("name", "N/A"),
                "email": coord.get("email", "N/A"),
                "state": coord.get("state", "N/A"),
                "phone_number": coord.get("phone_number", "N/A"),
                "location": coord.get("location", "N/A"),
                "status": coord.get("status", "pending"),
                "approval_status": coord.get("approval_status", "pending"),
                "registered_on": coord.get("created_at", "N/A")
            })
        
        report_data["summary"]["total_count"] = len(coordinators)
    
    elif input.report_type == "parents":
        # Get parents
        parents_query = {"role": "parent"}
        parents_query.update(date_query)
        
        parents = await db.users.find(parents_query, {"_id": 0, "password_hash": 0}).to_list(None)
        
        for parent in parents:
            # Get student count
            student_count = await db.students.count_documents({"parent_id": parent.get("id")})
            
            report_data["details"].append({
                "user_code": parent.get("user_code", "N/A"),
                "name": parent.get("name", "N/A"),
                "email": parent.get("email", "N/A"),
                "state": parent.get("state", "N/A"),
                "phone_number": parent.get("phone_number", "N/A"),
                "location": parent.get("location", "N/A"),
                "student_count": student_count,
                "registered_on": parent.get("created_at", "N/A")
            })
        
        report_data["summary"]["total_count"] = len(parents)
    
    return report_data

@api_router.post("/admin/reports/download")
async def download_report(input: ReportInput, request: Request, format: str = "csv"):
    """Download report as CSV or Excel"""
    await require_admin(request)
    
    # Generate report data
    report_response = await generate_report(input, request)
    details = report_response["details"]
    ...
    if format.lower() == "csv":
        ...
        response.headers["Content-Disposition"] = f"attachment; filename={input.report_type}_report_{input.from_date}_{input.to_date}.csv"
        return response
    ...
    # Generate Excel
        ...
        wb = Workbook()
        ws = wb.active
        ws.title = f"{input.report_type.capitalize()} Report"
        ...
        if details:
            ...
            # Add data rows
            for row_num, data_row in enumerate(details, 2):
                for col_num, header in enumerate(headers, 1):
                    ws.cell(row=row_num, column=col_num, value=str(data_row[header]))
        ...
        response.headers["Content-Disposition"] = f"attachment; filename={input.report_type}_report_{input.from_date}_{input.to_date}.xlsx"
        return response
...
async def run_migrations():
    ...
    try:
        ...
        # Password is read from ADMIN_PASSWORD env var (set on Render). Fallback allows
        # local/preview environments to work without setting the env var.
        admin_password = os.environ.get("ADMIN_PASSWORD", "RSN@Admin2026!")
